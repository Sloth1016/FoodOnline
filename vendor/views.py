...
def vprofile(request):
    ...
    if request.method == 'GET' and request.GET.get('action') == 'get_address_suggestions':
        uri = "/place/v2/suggestion"
        query = request.GET.get('query', '')
        region = '中国'  # 替换为默认城市
        ak = settings.BAIDU_API_KEY
        sk = settings.BAIDU_API_SECRET  # 添加你的 Secret Key
        ...
        if len(query) > 2:
            ...
            # 将 SN 参数添加到请求中
            url = f"https://api.map.baidu.com{query_str}&sn={sn}"
